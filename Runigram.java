...
public class Runigram {
	...
	public static Color[][] read(String fileName) {
		...
		for(int i = 0; i < numRows; i++){
			for(int j = 0; j< numCols; j++){
				int curr_r = in.readInt();
				int curr_g = in.readInt();
				int curr_b = in.readInt();
				image[i][j] = new Color(curr_r, curr_g, curr_b);
			}
		}
		return image;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		double h_ratio = (double)image.length / height;
		double w_ratio = (double)image[0].length / width;
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		for(int i = 0; i <= n; i++){
			...
			Color[][] temp = blend(source, target, alpha);
			display(temp);
			StdDraw.pause(500);
			source = temp;
		}
